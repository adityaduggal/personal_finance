...
from ...api.nse_tools.nse_api import get_stock_quote
from ..price_tracker.price_tracker import get_price_doc, update_stock_prices
...
class GetStockPrice(Document):
    ...
    def get_prices(self):
        ...
        elif self.type in ("Stock", "Index"):
            # From Date would be the date defined in settings or the date in the Item Master upto which
            # Prices are already fetched
            to_dt = getdate(self.date)
            if itd.prices_fetched_upto:
                frm_dt = getdate(itd.prices_fetched_upto)
            else:
                frm_dt = datetime.strptime("01-01-2000", '%d-%m-%Y').date()
            prices = get_stock_quote(stock=itd.symbol, frm_dt=frm_dt, to_dt=to_dt)
            if prices.empty:
                frappe.throw(f"No Prices fetched for {self.item_name} from date {frm_dt} to date {to_dt}")
            else:
                update_stock_prices(self.item_name, prices)
...
def update_all_prices(item_name):
    ...
    if itgd.type_of_group == "Mutual Fund" and itd.symbol:
        ...
        if prices.status == "SUCCESS":
            ...
            frappe.throw("No Prices Received")
    elif itgd.type_of_group == "Stock" and itd.symbol:
        to_dt = getdate(today())
        if itd.prices_fetched_upto:
            frm_dt = getdate(itd.prices_fetched_upto)
        else:
            frm_dt = datetime.strptime("01-01-2000", '%d-%m-%Y').date()
        prices = get_stock_quote(stock=item_name, frm_dt=frm_dt, to_dt=to_dt)
        if prices.empty:
            print(f"No Prices fetched for {self.item_name} from date {frm_dt} to date {to_dt}")
        else:
            update_stock_prices(item_name, prices)
